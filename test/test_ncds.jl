...
function test_ncds(file_name)
    ...
    effjointrelax =
        round.(Int, inst.nB * soljointrelax.jointYZB + inst.nA * soljointrelax.jointYZA)
    effgroup = round.(Int, inst.nB * solgroup.jointYZB + inst.nA * solgroup.jointYZA)
    effgrouprelax =
        round.(Int, inst.nB * solgrouprelax.jointYZB + inst.nA * solgrouprelax.jointYZA)
    ...
    println(
        "Error with relaxed group = ",
        1 / 2.0 * sum(abs.(tab .- effgrouprelax)) / sum(tab),
    )
    println("Error with  joint = ", 1 / 2.0 * sum(abs.(tab .- effjoint)) / sum(tab))
    println(
        "Error with relaxed joint = ",
        1 / 2.0 * sum(abs.(tab .- effjointrelax)) / sum(tab),
    )
